...
function DXF_DSS(caso)
    % read file and plot
    dxf = DXFtool('Estocolmo47.dxf');
    ...
    Buses1=0;
    f=3;
    [Gen_Max,Curva_dem]=Param_Iniciales;
    for i = 1:dxf.ne    
        ...
        if(strcmp(dxf.entities(i).layer,'Carga3f')==1 && strcmp(dxf.entities(i).name,'POINT')==1)
     ...
     for i =1:r
         ...
         if((abs(CT.point(1)-Buses(i,1))<=0.1) && (abs(CT.point(2)-Buses(i,2))<=1))
     ...
     for i =1:r 
         for j=1:r1
             if((abs(Loads(j,2)-Buses(i,1))<=0.1) && (abs(Loads(j,3)-Buses(i,2))<=1)) %%Define en cada bus si tiene una carga conectada escribiendo un 1 en la posición 3 del vector
                Buses(i,3)=Loads(j,4);            
             end             
         end
         Buses(i,4)=0; 
         ...
         if Buses(Lines(j,5),3)~=0 || Buses(Lines(j,6),3)~=0
            ...
            elseif Buses(Lines(j,5),3)==3 || Buses(Lines(j,6),3)==3
                 ...
                 elseif Buses(Lines(j,6),3)==3
                    ...
                    %%if mod(j,2)==0                %%Se crea un nuevo generador trifásico? en ese punto.
                       %% PV_generator(nombrebus2,Gen_Max/1000);            
                    %%end  
                    NewLoad(nombre,nombrebus2,3,35,Curva_dem); %Se supone que cada piso tiene contratada 3.5 kW y hay 10 casas por bloque
                    f=3;
            end
        else
         nombrebus2=sprintf("bus%d",Lines(j,6));
